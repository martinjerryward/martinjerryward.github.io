...
#define SPEED_CHANGE 0.1f  // Amount by which to increment or decrement speed
...
void limitSpeed(Boid* boid, float maxSpeed) {
    ...
    if (speed < MIN_SPEED) {  // Ensure speed doesn't go below the minimum
...
// Function to handle special key input (for Page Up and Page Down)
void specialInput(int key, int x, int y) {
    for (int i = 0; i < NUM_BOIDS; i++) {
        switch (key) {
        case GLUT_KEY_PAGE_UP:
            // Increase boid velocity incrementally
            boids[i].vx += (boids[i].vx > 0 ? SPEED_CHANGE : -SPEED_CHANGE);
            boids[i].vy += (boids[i].vy > 0 ? SPEED_CHANGE : -SPEED_CHANGE);
            break;
        case GLUT_KEY_PAGE_DOWN:
            // Decrease boid velocity incrementally (but ensure it doesn't go below min speed)
            boids[i].vx -= (boids[i].vx > 0 ? SPEED_CHANGE : -SPEED_CHANGE);
            boids[i].vy -= (boids[i].vy > 0 ? SPEED_CHANGE : -SPEED_CHANGE);
            break;
        }
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Boids - Incremental Speed Control");

    init();
    glutDisplayFunc(display);
    glutSpecialFunc(specialInput);  // Register the special key handler (for Page Up/Down)
